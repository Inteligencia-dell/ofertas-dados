...
def localizar_opcao_cidade(page, cidade, uf):
    """Localiza o botao do autocomplete pelo identificador sem acentos."""
    seletor = (
    ...
    candidatos = page.locator(
        "button[data-gtm-event-label^='alterar-cidade-']"
    )
    for indice in range(candidatos.count()):
        candidato = candidatos.nth(indice)
        try:
            ...
            cidade_ok = alvo_cidade in texto
            uf_ok = bool(re.search(
                r"(^|[\s/\-(])%s([\s/\-)]|$)" % re.escape(alvo_uf),
                texto,
            ))
            if cidade_ok and uf_ok:
                return candidato
        except Exception:
            continue
    return None
...
def selecionar_cidade(page, cidade, uf):
    ...
    label_padrao = (
        "lista-cidades-padrao-%s-%s"
        % (slug(cidade), uf.lower())
    )
    opcao = primeiro_visivel([
        page.locator(
            "button[data-gtm-event-label='%s']" % label_padrao
        ),
    ])

#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Piloto seguro da Claro por cidade para vinte localidades.
...
SAIDA = RAIZ / "dados" / "piloto-claro-20-cidades.json"
EVIDENCIAS = RAIZ / "evidencias-claro-20-cidades"
URL = "https://www.claro.com.br/"
TIMEOUT = 60000
ESPERA_CIDADE = 8000
ESPERA_VITRINE = 20000
INTERVALO_VITRINE = 1000

CIDADES = [
    {"city": "ADAMANTINA", "uf": "SP"},
    {"city": "AGUDOS", "uf": "SP"},
    {"city": "ALVARES MACHADO", "uf": "SP"},
    {"city": "AMERICANA", "uf": "SP"},
    {"city": "AMERICO BRASILIENSE", "uf": "SP"},
    {"city": "AMPARO", "uf": "SP"},
    {"city": "ANDRADINA", "uf": "SP"},
    {"city": "APARECIDA", "uf": "SP"},
    {"city": "ARACATUBA", "uf": "SP"},
    {"city": "ARARAQUARA", "uf": "SP"},
    {"city": "ARARAS", "uf": "SP"},
    {"city": "ARTUR NOGUEIRA", "uf": "SP"},
    {"city": "AVARE", "uf": "SP"},
    {"city": "BADY BASSITT", "uf": "SP"},
    {"city": "BARRETOS", "uf": "SP"},
    {"city": "BARRINHA", "uf": "SP"},
    {"city": "BATATAIS", "uf": "SP"},
    {"city": "BAURU", "uf": "SP"},
    {"city": "BEBEDOURO", "uf": "SP"},
    {"city": "BERTIOGA", "uf": "SP"},
]
...
def vitrine_tem_placeholder(page):
    secoes = page.locator("section.cms-Card360.has-title")
    for indice in range(secoes.count()):
        secao = secoes.nth(indice)
        try:
            texto = norm(secao.inner_text())
        except Exception:
            continue
        if "MELHORES OFERTAS DE CONECTIVIDADE PARA SUA CASA E SEU CELULAR" in texto:
            return "{{INTERNET." in texto
    return True


def aguardar_vitrine_residencial(page, timeout_ms=ESPERA_VITRINE):
    """Aguarda os cards dinamicos terem velocidade e preco reais."""
    limite = dt.datetime.now(dt.timezone.utc) + dt.timedelta(milliseconds=timeout_ms)
    while dt.datetime.now(dt.timezone.utc) < limite:
        if not vitrine_tem_placeholder(page):
            cards = page.locator("div.mdn-Card.mdn-Card--default")
            for indice in range(cards.count()):
                try:
                    texto = re.sub(r"\s+", " ", cards.nth(indice).inner_text()).strip()
                except Exception:
                    continue
                if re.match(r"^Fibra\b", texto, re.I) and re.search(r"\d{2,5}\s*Mega\b", texto, re.I):
                    if cards.nth(indice).locator(".mdn-Price-main-price").count() > 0:
                        return True
        page.wait_for_timeout(INTERVALO_VITRINE)
    return False
...
def consultar_cidade(browser, cidade, uf):
    ...
    try:
        ...
        if response and response.status >= 400:
            ...
            resultado["retryUsed"] = False
            resultado["dynamicDataLoaded"] = False

            if resultado["locationConfirmed"]:
                resultado["dynamicDataLoaded"] = aguardar_vitrine_residencial(page)

                if not resultado["dynamicDataLoaded"]:
                    resultado["retryUsed"] = True
                    page.reload(wait_until="domcontentloaded", timeout=TIMEOUT)
                    page.wait_for_timeout(4000)
                    texto = page.locator("body").inner_text()
                    resultado["locationConfirmed"] = local_confirmado(texto, cidade, uf)
                    if resultado["locationConfirmed"]:
                        resultado["dynamicDataLoaded"] = aguardar_vitrine_residencial(page)

                if resultado["dynamicDataLoaded"]:
                    resultado["offersFound"] = extrair_ofertas(page)

            if not ok:
                resultado["reason"] = erro
            elif not resultado["locationConfirmed"]:
                resultado["reason"] = "%s/%s nao confirmada no conteudo" % (cidade, uf)
            elif not resultado["dynamicDataLoaded"]:
                resultado["reason"] = "Cidade confirmada, mas dados dinamicos da vitrine nao carregaram apos retentativa"
...
def main():
    ...
    with sync_playwright() as p:
        ...
        browser.close()
    payload = {
        "meta": {"generated": agora(), "mode": "piloto_claro_20_cidades", "productionFilesChanged": False},
        "summary": {

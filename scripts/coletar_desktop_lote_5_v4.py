...
Modelo analitico preservado, identico ao aprovado em Campinas:
- fiberOffers
- fiberMobileOffers
- tvOffersAccepted
- locationConfirmed
...
Correcoes mantidas:
- filtro de cidade digitado com o nome oficial acentuado e recuo por prefixo;
- preco aceita qualquer vigencia, sem valor fixo assumido;
- promotionMonths registra a vigencia real da cidade.
"""
...
SAIDA = RAIZ / 'dados' / 'piloto-desktop-lote-5-v4.json'
EVID = RAIZ / 'evidencias-desktop-lote-5-v4'
URL = 'https://www.desktop.com.br/'
TIMEOUT = 60000

# Cada item traz a chave da base e o nome oficial acentuado usado no filtro do site.
CIDADES = [
    [
        "VALINHOS",
        "Valinhos"
...
def termos_de_busca(nome_oficial):
    """Gera o nome acentuado e prefixos decrescentes ate dois caracteres."""
...
def digitar_e_localizar(page, campo, chave, nome_oficial):
    """Digita o nome oficial e reduz o prefixo ate o filtro responder."""
    tentativas = []
...
def selecionar_cidade(page, chave, nome_oficial):
    ...
    motivo = None if confirmado else '%s/SP nao confirmada' % chave
...
def preco_atual(texto):
    """Le o preco vigente em qualquer formato de vigencia."""
    texto = re.sub(r'\s+', ' ', texto or '').strip()
    padroes = [
...
def oferta_fibra(texto):
    """Internet residencial, aceitando Desktop Play de cortesia."""
    if tem_tv(texto):
        return None
...
def oferta_convergente(texto):
    """Exige velocidade residencial, franquia movel, preco atual e preco posterior."""
    if tem_tv(texto):
        return None

...
def localizar_cards_publicos(page):
    """Localiza cards na pagina publica, sem usar as rotas /planos que retornaram 404."""
    seletores=[
        "article",
        "[class*='plan' i]",
        "[class*='offer' i]",
    ...
    for seletor in seletores:
        ...
        for i in range(q):
            ...
            try:
                ...
                if not re.search(r'R\$',texto,re.I):continue
            except Exception:continue
            chave=norm(texto)
            if chave in vistos or len(texto)>2200:continue
            vistos.add(chave); cards.append((c,texto))
    return cards


def extrair(page):
    fibra=[]; movel=[]; tv_rejeitadas=0
    for card,texto in localizar_cards_publicos(page):
        t=norm(texto)
        tem_tv=any(x in t for x in ['TV + INTERNET','DESKTOP PLAY','67 CANAIS','CANAIS EM HD','CANAIS AO VIVO'])
        if tem_tv:
            tv_rejeitadas+=1; continue
        ...
        if gbs:
            for gb in sorted(set(gbs)):
                movel.append({
                    ...
                    'name':f'{speed} Mega + Desktop Movel {gb} GB',
                    'fixedSpeedMb':speed,'mobilePlanType':'movel','mobileAllowanceGb':gb,
                    'monthlyPrice':atual,'postPromotionPrice':posterior,'tvIncluded':False,
                    'sourceSection':'internet_celular_publico','source':HOME,'cardText':texto[:900]
                })
        else:
            fibra.append({
                ...
                'tvIncluded':False,'sourceSection':'internet_publico','source':HOME,'cardText':texto[:900]
            })
...
def disponibilidade_campinas(page):
    texto=page.locator('body').inner_text()
    indisponivel=bool(re.search(r'Em breve estaremos na sua regi[aã]o',texto,re.I))
    cidade=bool(re.search(r'Campinas\s*/\s*SP',texto,re.I))
    return cidade and not indisponivel, indisponivel
...
def main():
    ...
    d={'operator':'Desktop','city':'CAMPINAS','uf':'SP','generated':agora(),'source':HOME,'httpStatus':None,'locationConfirmed':False,'citySelected':False,'cityAvailabilityConfirmed':False,'fiberOffers':[],'fiberMobileOffers':[],'tvOffersAccepted':0,'tvOffersRejected':0,'status':'a_validar','productionFilesChanged':False}
    try:
        with sync_playwright() as p:
            ...
            ok,erro=selecionar_campinas(page); d['citySelected']=ok
            disponivel,em_breve=disponibilidade_campinas(page)
            d['cityAvailabilityConfirmed']=disponivel
            d['locationConfirmed']=disponivel
            d['campinasComingSoon']=em_breve
            # Coleta o portfolio publico da home mesmo quando Campinas aparece como Em breve.
            d['fiberOffers'],d['fiberMobileOffers'],d['tvOffersRejected']=extrair(page)
            d['territorialScope']='campinas_confirmada' if disponivel else 'portfolio_publico_nao_disponivel_em_campinas'
            if em_breve:
                d['status']='campinas_em_breve_portfolio_publico'
                d['reason']='A Desktop informa Em breve estaremos na sua regiao para Campinas/SP; portfolio coletado sem atribuir disponibilidade municipal'
            elif disponivel and d['fiberOffers']:
                d['status']='validada_piloto'
            elif not ok:
                d['status']='cidade_nao_confirmada_portfolio_publico'
                d['reason']=erro or 'Campinas/SP nao confirmada'
            else:
                d['status']='cidade_confirmada_sem_fibra_extraida'
                d['reason']='Campinas confirmada, mas nenhuma oferta Internet foi extraida'

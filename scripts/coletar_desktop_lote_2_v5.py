...
Modelo analitico identico ao aprovado em Campinas:
- fiberOffers, fiberMobileOffers, tvOffersAccepted, locationConfirmed.

Correcao desta versao: troca de aba confirmada.
O script so aceita a aba Internet + Celular quando o conjunto de cards muda,
com novas tentativas e rotulos alternativos.
"""
...
SAIDA = RAIZ / 'dados' / 'piloto-desktop-lote-2-v5.json'
EVID = RAIZ / 'evidencias-desktop-lote-2-v5'
...
def assinatura_cards(cards):
    return '||'.join(sorted(c[:120] for c in cards))


def abrir_aba(page, rotulos, anterior=None, tentativas=3):
    """Clica na aba e confirma que o conjunto de cards realmente mudou."""
        ...
        for rotulo in rotulos:
            clicou = page.evaluate(r"""label => {
              ...
              const todos = [...document.querySelectorAll(
                'button,a,[role=tab],[role=button],li,span,div')].filter(vis);
              let el = todos.find(x => n(x.innerText || x.textContent) === alvo);
              if (!el) el = todos.find(x => {
                const t = n(x.innerText || x.textContent);
                return t.startsWith(alvo) && t.length <= alvo.length + 6;
              });
              if (!el) return false;
              const clicavel = el.closest('button,a,[role=tab],[role=button],li') || el;
              clicavel.scrollIntoView({block: 'center'});
              clicavel.click();
              return true;
            }""", rotulo)
            ...
            if atuais and (anterior is None or assinatura_cards(atuais) != anterior):
                return atuais
        page.wait_for_timeout(1500)
    return cards_visiveis(page)
...
def oferta_fibra(texto):
    ...
    return {
        ...
        'fixedSpeedMb': speed, 'monthlyPrice': atual, 'previousPrice': None,
        'postPromotionPrice': preco_posterior(texto),
        'promotionMonths': vigencia(texto),
        'desktopPlayIncluded': play, 'desktopPlayCourtesy': cortesia,
        'tvIncluded': False, 'cardText': texto[:900],
    }
...
def oferta_convergente(texto):
    ...
    return {
        ...
        'name': '%s + Movel %d GB' % ('1 Giga' if speed == 1000 else '%d Mega' % speed, gb),
        'fixedSpeedMb': speed, 'mobilePlanType': 'movel', 'mobileAllowanceGb': gb,
        'monthlyPrice': atual, 'previousPrice': None,
        'postPromotionPrice': preco_posterior(texto),
        'promotionMonths': vigencia(texto),
        'tvIncluded': False, 'cardText': texto[:900],
    }
...
def coletar_cidade(browser, chave, nome_oficial):
    ...
    try:
        ...
        if not ok:
            ...
            cards_internet = abrir_aba(page, ['Internet'])
            fibra = [x for x in (oferta_fibra(t) for t in cards_internet) if x]

            assinatura_internet = assinatura_cards(cards_internet)
            cards_celular = abrir_aba(
                page,
                ['Internet + Celular', 'Internet e Celular', 'Internet+Celular'],
                anterior=assinatura_internet,
            )
            convergentes = [x for x in (oferta_convergente(t) for t in cards_celular) if x]

            for texto in cards_internet:
                item = oferta_convergente(texto)
                if item:
                    convergentes.append(item)

            resultado['tabDiagnostics'] = {
                'internetCards': len(cards_internet),
                'mobileCards': len(cards_celular),
                'mobileTabChanged': assinatura_cards(cards_celular) != assinatura_internet,
            }
...
def main():
    ...
    dados = {'operator': 'Desktop', 'mode': 'lote_2_v5', 'generated': agora(),

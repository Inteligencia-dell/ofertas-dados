...
SAIDA = RAIZ / "dados" / "piloto-claro-fibra-convergente-campinas.json"
EVIDENCIAS = RAIZ / "evidencias-claro-fibra-convergente-campinas"
...
def extrair_ofertas_convergentes(page):
    """Extrai apenas Fibra + Movel; exclui Fibra + TV e Fibra + Movel + TV."""
    secoes = page.locator("section.cms-Card360.has-title")
    secao_multi = None
    for indice in range(secoes.count()):
        secao = secoes.nth(indice)
    ...
    for indice in range(cards.count()):
        ...
        # Obrigatorio: componente Fibra e componente movel no mesmo card.
        velocidade = extrair_velocidade(texto)
        movel = re.search(r"\b(Pos|P[oó]s|Controle)\s*(\d+)\s*GB\b", texto, re.I)
        if velocidade is None or not movel:
            continue

        # Exclusao expressa de qualquer composicao com TV.
        ...
        if preco_inteiro.count() == 0:
            continue
        preco = converter_preco(
            preco_inteiro.first.inner_text(),
            preco_centavos.first.inner_text() if preco_centavos.count() else "00",
        )
        if preco is None:
            continue
        ...
        tipo_movel = norm(movel.group(1)).lower()
        if tipo_movel == "pos":
            tipo_movel = "pos"
        elif tipo_movel == "controle":
            tipo_movel = "controle"
        ...
        for termo, descricao in [
            ("WI-FI", "Wi-Fi e instalação grátis"),
            ("STREAMING INCLUSO", "Streaming incluso"),
            ("WHATSAPP ILIMITADO", "WhatsApp ilimitado"),
...
def consultar_cidade(browser, cidade, uf):
    ...
    try:
        ...
        if response and response.status >= 400:
            ...
            resultado["convergentOffers"] = extrair_ofertas_convergentes(page) if resultado["locationConfirmed"] else []
            resultado["offersFound"] = resultado["fiberOffers"] + resultado["convergentOffers"]
            if not ok:
...
def main():
    ...
    with sync_playwright() as p:
        ...
        browser.close()
    payload = {
        "meta": {"generated": agora(), "mode": "piloto_claro_fibra_convergente_campinas", "productionFilesChanged": False},
        "summary": {
